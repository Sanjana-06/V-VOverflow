...
class TemplateRenderer {
    ...
    start_action() {
        if (!TemplateRenderer.templates[this.templatePath]) {
            ...
            return new Promise((resolve, reject) => resolve("Template in cache"));
        }
    }
    render() {
    ...
    doLinkReplacements(tag, attrib) {
        ...
        $.each(findersKeepers, (index, ele) => {
            let actualLink = $(ele).attr(attrib);
    ...
    static InitHandlebarHelpers() {
        ...
        Handlebars.registerHelper('isdefined', function (value) { return value !== undefined; });
